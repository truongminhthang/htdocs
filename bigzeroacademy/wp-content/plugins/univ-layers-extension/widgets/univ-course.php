...
if( class_exists('Layers_Widget') && !class_exists( 'univ_course_widget' ) ) {
	class univ_course_widget extends Layers_Widget {
		...
		*/
		function __construct() {
			...
			*/
			$this->widget_title = __( 'Univ:: Service' , 'univ' );
			$this->widget_id = 'bx-courses-widget-id';
			$this->post_type = '';
			$this->taxonomy = '';
			...
			$widget_ops = array(
				...
				'description' => __( 'This widget is used to display univ Course box in a flexible grid.', 'univ' ),
			...
			$this->defaults = array (
				'title' => __( 'Our Services', 'univ' ),
				'excerpt' => __( 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod.', 'univ' ),
				'design' => array(
					'layout' => 'layout-boxed',
					'liststyle' => 'list-grid',
					'gutter' => 'on',
			...
			$this->register_repeater_defaults( 'column', 3, array(
				'rovasticon' => __( 'paper-plane', 'univ' ),
				'title' => __( 'Your Course title', 'univ' ),
				'excerpt' => __( 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.', 'univ' ),
				'width' => '4',
				'design' => array(
					'coursesimage-size' => '',
		...
		*/
		function widget( $args, $instance ) {
			...
			if( !empty( $instance['design']['fonts'][ 'color' ] ) ) $this->inline_css .= layers_inline_styles( '#' . $widget_id, 'color', array( 'selectors' => array( '.section-title h3' , '.section-title p' ) , 'color' => $instance['design']['fonts'][ 'color' ] ) );
			...
			*/
			...
			$widget_container_class[] = 'univ-course-widget';
					...
					<div class="<?php echo $this->get_widget_layout_class( $instance ); ?> <?php echo $this->check_and_return( $instance , 'design', 'liststyle' ); ?>">
						<div class="grid">
							<?php foreach ( $column_ids as $column_key ) {
								...
								// Set Image Size
								if( isset( $item_instance['design']['coursesimage-size'] ) && 0 != $item_instance['design']['coursesimage-size'] && '' != $item_instance['design']['coursesimage-size'] ) {
									$image_width = $item_instance['design'][ 'coursesimage-size' ].'px';
								...
								*/
								...
								<div id="<?php echo $widget_id; ?>-<?php echo $column_key; ?>" class="<?php echo esc_attr( $classes ); ?>">
									...
									*/
									...
									$column_inner_classes[] = 'single-course-box';
									...
									<div class="<?php echo $column_inner_classes; ?>">
										...
										<?php if( $this->check_and_return( $item_instance, 'title' ) || $this->check_and_return( $item_instance, 'excerpt' ) || $this->check_and_return( $item_instance, 'rovasticon' ) || ( $link_array['link'] && $link_array['text'] ) ) { ?>
											<div class="media-body <?php echo ( isset( $item_instance['design']['fonts'][ 'align' ] ) ) ? $item_instance['design']['fonts'][ 'align' ] : ''; ?>">

												<?php if( $this->check_and_return( $item_instance, 'rovasticon' ) ) { ?>
													<div class="univ-icon-box"> <i class="fa fa-<?php layers_the_content( $item_instance['rovasticon'] ); ?>"></i></div>
												<?php } ?>
					...
					'liststyle' => array(
						...
						'elements' => array(
							'liststyle' => array(
								...
								'options' => array(
									'list-grid' => __( 'Grid' , 'univ' ),
									'list-masonry' => __( 'Masonry' , 'univ' )
								)
							),
							'gutter' => array(
								'type' => 'checkbox',
								'label' => __( 'Gutter' , 'univ' ),
				...
				<section class="layers-accordion-section layers-content">
					...
					<div class="layers-row">
						...
						<p class="layers-form-item">
							<label for="<?php echo $this->get_layers_field_id( 'rovasticon' ); ?>"><?php _e( 'Icon' , 'univ' ); ?></label>
							<?php echo $this->form_elements()->input(
								array(
									'type' => 'text',
									'name' => $this->get_layers_field_name( 'rovasticon' ),
									'id' => $this->get_layers_field_id( 'rovasticon' ),
									'placeholder' => __( 'Enter Icon Name' , 'univ' ),
									'value' => ( isset( $item_instance['rovasticon'] ) ) ? $item_instance['rovasticon'] : NULL ,
									'class' => 'layers-text'
								)
							); ?>
						</p>
						...
						<p class="layers-form-item">
							<label for="<?php echo $this->get_layers_field_id( 'title' ); ?>"><?php _e( 'Title' , 'univ' ); ?></label>
							<?php echo $this->form_elements()->input(
								array(
									'type' => 'text',
									'name' => $this->get_layers_field_name( 'title' ),
	...
	// Add our function to the widgets_init hook.
	 register_widget("univ_course_widget");
}