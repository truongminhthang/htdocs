 ...
 * @since Layers 1.0.0
 */
if( !class_exists( 'Univ_Contact_Widget' ) ) {
	class Univ_Contact_Widget extends Layers_Widget {
		...
		*/
	 	function __construct() {
			...
			*
		 	...
		 	*/
			$this->widget_title = __( 'Univ:: Contact' , 'univ' );
			$this->widget_id = 'univ_contact';
			$this->post_type = '';
			$this->taxonomy = '';
			...
			$widget_ops = array(
				...
				'description' => __( 'This widget is used to display your ', 'univ' ) . $this->widget_title . '.',
		...
		*/
	 	function widget( $args, $instance ) {
			...
			*/
			...
			$widget_container_class[] = 'univ-contact-wrapper';
					...
					'display' => array(
						...
						'elements' => array(
							...
							'google_maps_zoom' => array(
								...
								'label' => __( 'Google Map Zoom Level' , 'univ' ),
								'options' => array(
									'16' => __( 'Close', 'univ' ),
									'14' => __( 'Default', 'univ' ),
									'12' => __( 'Far', 'univ' ),
							...
							'show_contact_form' => array(
								...
								'label' => __( 'Show Contact Form' , 'univ' ),
							),
						),
					),
					'background',
				...
				<section class="layers-accordion-section layers-content">
					...
					<div class="layers-row layers-push-bottom clearfix">
						...
						<p class="layers-form-item">
							<label for="<?php echo $this->get_layers_field_id( 'google_maps_location' ); ?>"><?php _e( 'Google Maps Location' , 'univ' ); ?></label>
							<?php echo $this->form_elements()->input(
								array(
									'type' => 'text',
									'name' => $this->get_layers_field_name( 'google_maps_location' ) ,
									'id' => $this->get_layers_field_id( 'google_maps_location' ) ,
									'placeholder' => __( 'e.g. 300 Prestwich Str, Cape Town, South Africa' , 'univ' ),
									'value' => ( isset( $instance['google_maps_location'] ) ) ? $instance['google_maps_location'] : NULL
								)
							); ?>
						</p>
						<p class="layers-form-item">
							<label for="<?php echo $this->get_layers_field_id( 'google_maps_long_lat' ); ?>"><?php _e( 'Google Maps Latitude & Longitude (Optional)' , 'univ' ); ?></label>
							<?php echo $this->form_elements()->input(
								array(
									'type' => 'text',
									'name' => $this->get_layers_field_name( 'google_maps_long_lat' ) ,
									'id' => $this->get_layers_field_id( 'google_maps_long_lat' ) ,
									'placeholder' => __( 'e.g. -34.038181, 18.363826' , 'univ' ),
						...
						<p class="layers-form-item layers-push-top">
							<label for="<?php echo $this->get_layers_field_id( 'address_shown' ); ?>"><?php _e( 'Address Shown' , 'univ' ); ?></label>
							<?php echo $this->form_elements()->input(
								array(
									'type' => 'rte',
									'name' => $this->get_layers_field_name( 'address_shown' ) ,
									'id' => $this->get_layers_field_id( 'address_shown' ) ,
									'placeholder' => __( 'e.g. Prestwich Str, Cape Town' , 'univ' ),
					...
					<div class="layers-row clearfix">
						<p class="layers-form-item">
							...
							<label for="<?php echo $this->get_layers_field_id( 'contact_form' ); ?>"><?php _e( 'Contact Form Embed Code' , 'univ' ); ?></label>
							...
							<?php echo $this->form_elements()->input(
								array(
									...
									'placeholder' =>  __( 'Contact form embed code' , 'univ' ),
									'value' => ( isset( $instance['contact_form'] ) ) ? $instance['contact_form'] : NULL ,
									'class' => 'layers-textarea'
								)
							); ?>
							<small class="layers-small-note">
								<?php _e( sprintf( 'Need to create a contact form? Try <a href="%1$s" target="ejejcsingle">Gravity Forms</a>', 'https://www.e-junkie.com/ecom/gb.php?cl=54585&c=ib&aff=221037' ) , 'univ' ); ?>
							</small>
						</p>
					</div>
				</section>
	...
	// Add our function to the widgets_init hook.
	 register_widget("Univ_Contact_Widget");
}